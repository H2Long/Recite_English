...
//
// 设计理念：
//   采用"立即模式（Immediate Mode）"UI 风格 — 每一帧都重新检测交互状态，
//   不使用传统的保留模式（Retained Mode）UI 框架。优点是无需维护控件实例，
//   缺点是每帧都要重新执行绘制逻辑。
//
// 混合字体渲染：
//   本文件包含 UIGetFontForChar() 等函数，根据字符的 Unicode 码点自动选择
//   中文字体/英文字体/IPA 音标字体，实现同一文本中多字体混合渲染。
//
// UTF-8 完整支持：
//   所有文本处理（输入框、标签等）都完整支持 UTF-8 编码，可正确显示
//   和处理中文、日文等双字节/三字节字符。实现了 utf8_strlen()、
//   utf8_insert_codepoint()、utf8_delete_left() 等完整的 UTF-8 工具函数。
//
// 依赖关系：
//   - raylib_word_ui.h: 类型定义和函数声明
//   - fonts.c (extern): MeasureTextAuto 用于混合文本宽度测量
//   - C 标准库: string.h / stdio.h / math.h / stdlib.h
// ============================================================================
