...
void MenuSelectWord_Show(void) {
    ...
    // 答题完成
    if(SELECT_WORD.currentSelectIdx >= SELECT_WORD.selectCount && SELECT_WORD.selectTotal > 0) {
        ...
        DrawTextAuto(u8"练习完成！", (Vector2){SCREEN_WIDTH/2 - ts.x/2, rr.y + 35},
            46, 1, STYLE->theme.primary);

        float score = (float)SELECT_WORD.selectCorrect / SELECT_WORD.selectTotal * 100;
        char st[128];
        snprintf(st, sizeof(st), u8"正确率: %.0f%% (%d/%d)",
    ...
    // 正在答题
    int wi = SELECT_WORD.selectIndices[SELECT_WORD.currentSelectIdx];
    ...
    DrawTextAuto(u8"请选择正确的英文单词", (Vector2){SCREEN_WIDTH/2 - 160, qr.y + 12},
        20, 1, STYLE->theme.textSecondary);
    UIDrawTextRec(cw->definition, (Rectangle){qr.x + 25, qr.y + 40, qr.width - 50, 65},
        32, 1, true, STYLE->theme.textPrimary);
